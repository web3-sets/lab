...
import { filterTransactionsUsingSmartContractSet, isEVMStatePassingSetValidation } from '@web3-sets/runtime-engine'
...
export function FormRunSetValidation({ schema }: FormRunSetValidationProps) {
  ...
  const onSubmit = async (data: any) => {
    ...
    console.log(schema, transactions, 'schema, transactions')
    // @ts-ignore
    const isPassing = await isEVMStatePassingSetValidation(state, schema)
    const filteredTx = await filterTransactionsUsingSmartContractSet(schema, transactions)
    updateSet(schema?.id, {
      analytics: {
        isRun: true,
        isPassing,
